...
const inputCls = `bg-black/20 border border-white/10 text-white rounded-lg px-3 py-2.5 text-sm
  focus:outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-500/40 transition-all w-full`;
...
export default function Scorer() {
  ...
  return (
    <section className="animate-fade-up space-y-6">
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 mb-7">
            <Field label="Step (hour)">
              <input type="number" min={1} value={form.step} onChange={set('step')} className={inputCls} required />
            </Field>
            ...
            <Field label="Amount ($)">
              <input type="number" step="0.01" min={0} value={form.amount} onChange={set('amount')} className={inputCls} required />
            </Field>

            <Field label="Originator Account">
              <input type="text" value={form.nameOrig} onChange={set('nameOrig')} className={inputCls} required />
            </Field>

            <Field label="Old Balance (Orig)">
              <input type="number" step="0.01" min={0} value={form.oldbalanceOrg} onChange={set('oldbalanceOrg')} className={inputCls} required />
            </Field>

            <Field label="New Balance (Orig)">
              <input type="number" step="0.01" min={0} value={form.newbalanceOrig} onChange={set('newbalanceOrig')} className={inputCls} required />
            </Field>

            <Field label="Destination Account">
              <input type="text" value={form.nameDest} onChange={set('nameDest')} className={inputCls} required />
            </Field>

            <Field label="Old Balance (Dest)">
              <input type="number" step="0.01" min={0} value={form.oldbalanceDest} onChange={set('oldbalanceDest')} className={inputCls} required />
            </Field>

            <Field label="New Balance (Dest)">
              <input type="number" step="0.01" min={0} value={form.newbalanceDest} onChange={set('newbalanceDest')} className={inputCls} required />
            </Field>
          </div>
      ...
      {/* Result card */}
      {result && (
        <div className="glass p-7 animate-fade-up">
          <h3 className="text-xl font-bold mb-6">🎯 Prediction Result</h3>
          ...
          <div className="grid grid-cols-3 gap-4 mb-2">
            ...
            {/* Risk Tier */}
            <div className="bg-black/20 rounded-xl p-5 text-center flex flex-col items-center justify-center gap-2">
